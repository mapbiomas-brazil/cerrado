...
filtered2 <- ee$FeatureCollection('users/dh-conciani/collection7/sample/points/samplePoints_v6')
...
for (i in 1:length(unique(regions_list))) {
  ...
  ## extract spectral signatures
   print('extracting filtered GEDI/RAW')
  ## extract signatures
  ...
  ## merge datasets
  training <- as.data.frame(filtered2_training)
  rm(filtered2_training)
...
## export data
write.csv(recipe, './table/acc_versions_gedi_raw.csv')
